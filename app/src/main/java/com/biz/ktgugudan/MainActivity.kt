...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Change Envent 를 가로채서 글자를 view에 보이기를 해 보자
        ...
        // 아무런 메시지가 없을떄 문자열 "0"으로 초기화하여 클래스를 객체를 생성하라
            ...
            override fun beforeTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) {
                ...
                subject.map { txt.text.toString() == "" } // 최초에 app이 실행되거나,
                    ...
                    // java Stream(연속된 데이터) 형태의 자료를 생성하는 method
                        // flaMap에 의해 생성된 데이터는 Observable형태의 데이터이다.
                        // Observable관련된 method등을 chain하여 데이터를 처리할 수 있다

                    .flatMap({BehaviorSubject.range(1,9) }) // {1,2 -9} 형태의 배열자료를 ㅁ나들어라
                    {_,row -> "0 x $row = 0\n"}
                    .scan {x,y -> x+y}
                 ...
                 * 그 코드에 의해 문자열이 모두 생성되어서 화면에 나타날떄까지 실제로 mainActivity의
                 * 다른 코드나 이벤트의 실행에 영향을 미치게 된다.
                 *
